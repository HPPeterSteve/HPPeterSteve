...
## `$ cat ~/projects/others.txt`

```bash
pedro@linux:~/projects$ ls

📁 experimentos/     → testes com Linux internals & syscalls
📁 ferramentas/      → scripts e utilitários do dia a dia
📁 estudos/          → sistemas operacionais & segurança
📁 ?????/            → [em desenvolvimento... às 3h da manhã]
```
...
## `$ cat /etc/peter/fun-facts`

```python
peter = {
    "OS"        : "Linux (obviously)",
    "editor"    : "VSCode (Neovim me persegue nos sonhos)",
...
```
[pedro@linux ~]$ echo "obrigado pela visita"
